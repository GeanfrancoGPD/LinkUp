...
class LinkBO {
    repository;
    constructor(repository = LinkRepository_js_1.default) {
    ...
    async enviarSolicitud(req, res) {
        try {
            ...
            // Validación de input
            if (id_usuario_recibe === undefined ||
    ...
    async listarSugerencias(req, res) {
        try {
    ...
    async createChat(req, res) {
        try {
            ...
            if (id_usuario_destino === undefined ||
                id_usuario_destino === null ||
                typeof id_usuario_destino !== "number") {
                return res.status(400).json({
                    success: false,
                    message: "El campo id_usuario_destino es obligatorio y debe ser numérico",
                });
            }
            ...
            const existingChat = await this.repository.obtenerChatPrivadoEntreUsuarios(id_usuario, id_usuario_destino);
            if (existingChat) {
                return res.status(409).json({
                    success: false,
    ...
    async getChatsPorUsuario(req, res) {
        try {
            const id_usuario = this.resolveSessionUserId(req);
    ...
    async eliminarChat(req, res) {
        try {
            ...
            const chatExists = await this.repository.obtenerChatUsuario(this.resolveSessionUserId(req), id_chat);
            if (!chatExists) {
                return res.status(404).json({
                    success: false,
    ...
    async obtenerHistorialMensajes(req, res) {
        try {
            ...
            const esParticipante = await this.repository.esParticipanteChat(id_chat, id_usuario);
            if (!esParticipante) {
            ...
            const mensajes = await this.repository.getMensajesChatPaginados(id_chat, before_id, limit);
            return res.json({
                success: true,
                data: mensajes,
    ...
    // ==========================================
    async procesarMensaje(input) {
        try {
            const esParticipante = await this.repository.esParticipanteChat(input.id_chat, input.id_usuario);
            if (!esParticipante) {
                return {
                    success: false,
    ...
    async validarParticipante(id_chat, id_usuario) {
        return await this.repository.esParticipanteChat(id_chat, id_usuario);
    }
}
exports.default = new LinkBO();
//# sourceMappingURL=LinkBO.js.map